    ...
    if ($con) {
        ...
        for ($i = 0; $i < count($objData); $i++) {
            ...
            $query = $con->prepare('SELECT vh.idvaloracion_hecha,IF(cev.estado is null, "creado", cev.estado) as estado,cev.fecha as ultimocambio  '
                    ...
                    . 'left join (select idcambio_estado_valoracion,estado,fecha,fk_valoracion_hecha_idvaloracion_hecha from checkpoint.cambio_estado_valoracion order by fecha desc limit 1) cev on cev.fk_valoracion_hecha_idvaloracion_hecha=vh.idvaloracion_hecha '

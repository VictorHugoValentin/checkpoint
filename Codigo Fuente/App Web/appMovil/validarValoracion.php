	...
	try {
		...
		if($con){
			...
			if($result["habilitado"] == 1) {
				$query = $con->prepare('SELECT  habilitado  FROM valoraciones WHERE idvaloraciones = "'.$idvaloracion).'"'; 	// <---- Cambiar Tabla de "Valoraciones" a "Valoracion"
																														// <---- MANDAR EL CODIGO NO EL NOMBRE
				$query->execute();
				if($result["habilitado"] == 1) {
